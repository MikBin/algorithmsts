import { buildSegmentTree, queryRange, buildSegTreeIterative, iterativeQueryRange, updateLeafNode, updateLeafNodeIterative, SegmentTree } from "../../src/segmentTree/segmentTree";
import { baseSegmentTreeNode } from "../../src/segmentTree/interfaces";
import { finNodeFactory, finNodeLeafUpdater, finNodeMerger, finNodeQuery } from "../../src/segmentTree/segmentTree"
...
describe("testing segment tree class: ", () => {
  let FinSegmentTree = new SegmentTree(financialSeries, finNodeFactory, finNodeMerger, finNodeQuery, finNodeLeafUpdater);
  ...
  const getSigma = (arr: (string | number)[][], mu: number, l: number = 0, r: number = arr.length) => {
    let s = 0, i = 0;
    for (i = l; i < r; i++) {
  ...
  const getMaxInRange = (arr: (string | number)[][], l: number = 0, r: number = arr.length) => {
    let m: number = <number>arr[l][2], i = 0;
    for (i = l; i < r; i++) {
  ...
  it("have full average correct", () => {
    ...
    let highs = financialSeries.map((v): number => { return <number>v[2]; });
    console.log(res);
