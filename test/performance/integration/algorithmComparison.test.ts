...
describe('Algorithm Comparison Integration Tests', () => {
  describe('Sorting Algorithm Comparison', () => {
    it('should compare CountingSort vs RadixSortNumbers performance', () => {
      ...
      const result = BenchmarkRunner.compareAlgorithms(
        countingSort,
        radixSort,
        { array: [...testArray] },
        10 // iterations
      ...
      expect(result.metrics1.executionTime).toBeGreaterThanOrEqual(0);
      expect(result.metrics2.executionTime).toBeGreaterThanOrEqual(0);
    ...
    it('should benchmark sorting algorithms with different input sizes', () => {
      ...
      const results = BenchmarkRunner.benchmarkAlgorithm(
        countingSort,
