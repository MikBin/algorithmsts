...
describe('AdjacencyListGraph', () => {
  ...
  describe('Weighted Graphs', () => {
    let weightedGraph: AdjacencyListGraph<string, number>;

    beforeEach(() => {
      weightedGraph = new AdjacencyListGraph<string, number>(false, true);
    ...
    it('should throw error when adding unweighted edge with weight', () => {
      const unweightedGraph = new AdjacencyListGraph<string>(false, false);
  ...
  describe('Integration with Sample Graphs', () => {
    it('should work with simple directed graph', () => {
      const testGraph = SampleGraphs.simpleDirectedGraph;
    ...
    it('should work with cyclic graph', () => {
      const testGraph = SampleGraphs.cyclicGraph;
      expect(testGraph.getVertexCount()).toBe(3);
      expect(testGraph.getEdgeCount()).toBe(3);
      expect(testGraph.getNeighbors('A')).toEqual(['B']);
      expect(testGraph.getNeighbors('B')).toEqual(['C']);
      expect(testGraph.getNeighbors('C')).toEqual(['A']);
    });
  });
