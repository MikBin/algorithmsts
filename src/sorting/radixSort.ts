...
/**@TODO to sort mixed negatives:
 * filter negatives sort separatedly map(math.abs) then reverse the array change sign and append to positives sorted
 */
export const radixSortNumbers = (arr: number[]): number[] => {
  let arrayToSort = arr
  let max = 0
  for (let i = 0; i < arr.length; i++) max = Math.max(arr[i], max)
